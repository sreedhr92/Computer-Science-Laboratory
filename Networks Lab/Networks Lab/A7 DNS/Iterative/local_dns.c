...
int main(int argc,char **argv)
{
    ...
    while(1)
    {
        ...
        {
            printf("\n**The Ip is not available in the Local DNS redirecting to Root DNS..**\n");
            p = 3000;
            a:
            serverAddr.sin_port=htons(p); //connecting to the respective ports
            ret = bind(sockfd,(struct sockaddr*)&serverAddr,sizeof(serverAddr));
