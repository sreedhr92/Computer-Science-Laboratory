...
int main(int argc,char **argv)
{
    ...
    while(1)
    {
        ...
        {
            printf("\n**The Ip is not available in the Root DNS redirecting to TLD server..**\n");
            serverAddr.sin_port=htons(2000);//connecting to the port tld_server.
            ret = bind(sockfd,(struct sockaddr*)&serverAddr,sizeof(serverAddr));
            sendto(sockfd,buffer,sizeof(buffer),0,(struct sockaddr *)&clientAddr,len);
            no = recvfrom(sockfd,buffer,sizeof(buffer),0,(struct sockaddr*)&clientAddr,&len);
            update_table(t,buffer,domain);
            printf("\nUpdated the table\n");
        }
        else
